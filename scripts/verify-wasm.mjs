#!/usr/bin/env node
// CI byte-identical reproducibility check for ruby.wasm.
...
const committed = await readFile(path.join(repoRoot, "ruby.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed ruby.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-ruby-verify-"));
...
console.log(`rebuilt ruby.wasm sha256: ${rebuiltHash}`);
