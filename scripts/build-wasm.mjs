#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-ruby.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-ruby-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-ruby.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as ruby.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "ruby.wasm"));
const bytes = (await readFile(path.join(repoRoot, "ruby.wasm"))).length;
console.log(`ruby.wasm: ${bytes} bytes (built from ${pin})`);
